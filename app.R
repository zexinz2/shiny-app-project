...
compute_dhlrc <- function(edge_df, node_deg_df, node_nb_df, edge_flow_df,
  ...
  result <- data.frame(
    edge_id = integer(), tail = character(), head = character(),
    dHLRC_original = numeric(), dHLRC_normalized = numeric(),
    gap = numeric(), tail_size = integer(), head_size = integer(),
    metric_type = character(), 
    node_term_original = numeric(), node_term_normalized = numeric(), 
    C_original = numeric(), C_normalized = numeric(),
    M_e = numeric(), m_e = numeric(),
    stringsAsFactors = FALSE
  )
  ...
  for (i in seq_len(nrow(edge_df))) {
    ...
    node_sum <- sum(1 / tail_safe) + sum(1 / head_safe)
    node_term_original <- node_sum                  
    node_term_normalized <- 0.5 * node_sum         
    ...
    dhlrc_original <- node_term_original + C_original / M_e + C_original / m_e - 1
    dhlrc_normalized <- node_term_normalized + C_normalized / M_e + C_normalized / m_e - 1
    ...
    result <- rbind(
      ...
      data.frame(
        ...
        tail_size = tail_size, head_size = head_size, metric_type = metric_type,
        node_term_original = node_term_original,      
        node_term_normalized = node_term_normalized,  
        C_original = C_original, C_normalized = C_normalized,
        M_e = M_e, m_e = m_e,
        stringsAsFactors = FALSE
      )
    )
